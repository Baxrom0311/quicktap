...
app.get("/api/leaderboard/:difficulty", async (req, res) => {
  try {
    ...
    // Get top scores and total concurrently. DISTINCT ON can use the
    // composite index from schema.sql and avoids a full window-sort per row.
      ...
      pool.query(
        `SELECT id, username, avatar, score, created_at,
           ...
           FROM (
             ...
             ORDER BY user_id, score ASC, created_at ASC
           ) best_scores
      ...
      `WITH user_best AS (
             ...
             SELECT
               ...
               CASE
                 ...
                 ELSE COUNT(ranked_best.user_id) FILTER (WHERE ranked_best.best_score < user_best.best_score) + 1
               END AS rank
             FROM user_best
             LEFT JOIN ranked_best ON user_best.best_score IS NOT NULL
             GROUP BY user_best.best_score`,
      [userId, difficulty]
    );

    res.json({
      rank: rankResult.rows[0].rank,
      best_score: rankResult.rows[0].best_score,
    });
  } catch (error) {
...
httpServer.listen(port, () => {
  log("info", `QuickTap API server running on port ${port}`, {
    port,
